...
def doubly_robust(df, X, Y):
    ...
    # model for propensity score
    pi = LogisticRegression(C=1e6, max_iter=1000).fit(df[X], df[DELTA]).predict_proba(Delta_1[X])[:, 1]
    
    # regression model trained on observed outcomes
    s = LinearRegression().fit(Delta_1[X], Delta_1[Y]).predict(Delta_1[X])

    mu_dr = np.mean(s + (Delta_1[Y]-s) * (1 / pi))
    return mu_dr
...
if __name__ == "__main__":
    ...
    # apply MAR mask   
    M = MAR_logistic(data_with_categ, 0.5, Y, None) 
    ...
    print('Horvitz-Thompson Esimator: {}'.format(horvitz_thompson(data_with_categ, X, Y)))
    print('Outcome Regression Esimator: {}'.format(outcome_regression(data_with_categ, X, Y)))
    print('Doubly Robust Esimator: {}'.format(doubly_robust(data_with_categ, X, Y)))