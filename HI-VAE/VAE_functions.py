...
def place_holder_types(types_file, batch_size):
    ...
    for i in range(len(types_list)):
        batch_data_list.append(tf.compat.v1.placeholder(tf.float32, shape=(batch_size, int(types_list[i]['dim']))))
    ...
    for i in range(len(types_list)):
        batch_data_list_observed.append(tf.compat.v1.placeholder(tf.float32, shape=(batch_size, int(types_list[i]['dim']))))
    tf.concat(batch_data_list_observed, axis=1)
        
    #Create placeholders for the missing data indicator variable
    miss_list = tf.compat.v1.placeholder(tf.int32, shape=(batch_size,len(types_list)))
    
    #Placeholder for Gumbel-softmax parameter
    tau = tf.compat.v1.placeholder(tf.float32,shape=())
    tau2 = tf.compat.v1.placeholder(tf.float32,shape=())
...
def s_proposal_multinomial(X, batch_size, s_dim, tau, reuse):
    ...
    U = -tf.math.log(-tf.math.log(tf.compat.v1.random_uniform([batch_size,s_dim])))
...
#                         kernel_initializer=tf.random_normal_initializer(stddev=0.05), name='layer_0_' + 'mean_enc_z', reuse=reuse)
    ...
    # Rep-trick
    eps = tf.compat.v1.random_normal((batch_size, z_dim), 0, 1, dtype=tf.float32)
